...
def test_published_site_dir_escapes_root(mock_root, tmp_path):
    ...
    with (
        ...
        pytest.raises(ValueError, match="Published site path escapes root"),
    ):  # noqa: E501
...
def test_validate_tree_invalid_widgets():
    ...
    assert _validate_tree({"sections": [{"columns": [{"widgets": "not a list"}]}]}) == {
        "sections": []
    }  # noqa: E501
...
def test_validate_tree_missing_ids():
    ...
    tree = {
        "sections": [
            {"columns": [{"widgets": [{"type": "heading", "props": {"text": "Hi", "level": 1}}]}]}
        ]
    }
    cleaned = _validate_tree(tree)
    assert len(cleaned["sections"]) == 1
