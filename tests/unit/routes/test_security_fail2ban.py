...
async def test_fail2ban_enable_exception(app, mock_admin, monkeypatch):
    import asyncio
    monkeypatch.setattr(asyncio, "create_subprocess_exec", AsyncMock(side_effect=Exception("Test Error")))
...
async def test_fail2ban_config_unauthorized(app, mock_no_admin):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        resp = await ac.post("/security/fail2ban/config/sshd", data={"bantime": 3600, "findtime": 600, "maxretry": 5})
        assert resp.status_code == 401

@pytest.mark.asyncio
async def test_fail2ban_config_invalid_jail(app, mock_admin):
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        resp = await ac.post("/security/fail2ban/config/inv jail", data={"bantime": 3600, "findtime": 600, "maxretry": 5})
        assert resp.status_code == 400
        assert "Invalid jail name" in resp.text
...
async def test_fail2ban_config_value_error(app, mock_admin, monkeypatch):
    monkeypatch.setattr(sf2b, "_save_jail_config", AsyncMock(side_effect=ValueError("Invalid config values")))
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        resp = await ac.post("/security/fail2ban/config/sshd", data={"bantime": 3600, "findtime": 600, "maxretry": 5})
        assert resp.status_code == 400
