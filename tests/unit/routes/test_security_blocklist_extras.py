...
def test_blocklist_page_unauth(monkeypatch):
    ...
    app.include_router(router)
    c = TestClient(app)
    monkeypatch.setattr("pit_panel.web.routes.security_blocklist.get_admin", AsyncMock(return_value=None))
...
def test_blocklist_import_no_ips(client, monkeypatch):
    monkeypatch.setattr("pit_panel.web.routes.security_blocklist.fetch_blocklist", AsyncMock(return_value=[]))
...
def test_blocklist_import_success(client, monkeypatch):
    monkeypatch.setattr("pit_panel.web.routes.security_blocklist.fetch_blocklist", AsyncMock(return_value=["1.1.1.1", "2.2.2.2"]))
    monkeypatch.setattr("pit_panel.web.routes.security_blocklist.ban_ips_bulk", AsyncMock(return_value=2))
