...
def test_repository_input_has_accessible_name():
    template = (
        Path(__file__).resolve().parents[4] / "src/pit_panel/web/templates/apps.html"
    ).read_text(encoding="utf-8")
    repo_input = next(tag for tag in template.split("<input") if 'id="repo-url"' in tag)
    assert 'aria-label="GitHub Repository URL"' in repo_input.split(">", 1)[0]
