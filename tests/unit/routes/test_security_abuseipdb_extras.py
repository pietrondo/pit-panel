...
def test_abuseipdb_blacklist_unauth(monkeypatch):
    ...
    c = TestClient(app)
    monkeypatch.setattr(
        "pit_panel.web.routes.security_abuseipdb.get_admin", AsyncMock(return_value=None)
    )
...
def test_abuseipdb_blacklist_no_key(client, monkeypatch):
    monkeypatch.setattr(
        ...
        MagicMock(return_value=MagicMock(abuseipdb_api_key="")),
    )
...
def test_abuseipdb_check_score_colors(client, monkeypatch):
    monkeypatch.setattr(
        ...
        AsyncMock(return_value={"ip": "1.1.1.1", "score": 10, "reports": 0}),
    )
    ...
    monkeypatch.setattr(
        "pit_panel.web.routes.security_abuseipdb._abuseipdb_check",
        AsyncMock(return_value={"ip": "2.2.2.2", "score": 50, "reports": 0}),
    )
    ...
    monkeypatch.setattr(
        "pit_panel.web.routes.security_abuseipdb._abuseipdb_check",
        AsyncMock(return_value={"ip": "3.3.3.3", "score": 90, "reports": 0}),
    )
...
def test_abuseipdb_blacklist_http_client(monkeypatch):
    ...
    mock_resp.read.return_value = (
        b'{"data": [{"ipAddress": "1.1.1.1", '
        b'"abuseConfidenceScore": 100, "totalReports": 10, "lastReportedAt": "2023"}]}'
    )
    mock_conn.getresponse.return_value = mock_resp
