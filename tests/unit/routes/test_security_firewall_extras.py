...
def test_enable_firewall_fail(client, monkeypatch):
    monkeypatch.setattr(
        "pit_panel.web.routes.security_firewall._detect_ssh_port", AsyncMock(return_value=22)
    )
    monkeypatch.setattr(
        "pit_panel.web.routes.security_firewall._enable_ufw", AsyncMock(return_value=False)
    )
...
def test_disable_firewall_unauth(monkeypatch):
    ...
    c = TestClient(app)
    monkeypatch.setattr(
        "pit_panel.web.routes.security_firewall.get_admin", AsyncMock(return_value=None)
    )
...
def test_disable_firewall_fail(client, monkeypatch):
    monkeypatch.setattr(
        "pit_panel.web.routes.security_firewall._disable_ufw", AsyncMock(return_value=False)
    )
...
def test_add_rule_invalid_protocol(client):
    response = client.post(
        "/security/firewall/rule/add", data={"port": "80", "protocol": "invalid"}
    )
    assert response.status_code == 400
...
def test_add_rule_invalid_source(client):
    response = client.post(
        "/security/firewall/rule/add", data={"port": "80", "source": "invalid_ip"}
    )
    assert response.status_code == 400
...
def test_add_rule_fail(client, monkeypatch):
    monkeypatch.setattr(
        "pit_panel.web.routes.security_firewall._add_ufw_rule", AsyncMock(return_value=False)
    )
...
def test_delete_rule_fail(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "pit_panel.web.routes.security_firewall._delete_ufw_rule", AsyncMock(return_value=False)
    )
...
def test_delete_rule_value_error(client, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(side_effect=ValueError("Cannot delete SSH rule")),
    )
