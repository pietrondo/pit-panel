...
## 2026-09-06 - Accessible File Uploads
**Learning:** Using `display: none` (`class="hidden"`) on `<input type="file">` breaks keyboard and screen reader accessibility as it completely removes the element from the accessibility tree and focus order.
**Action:** Always use `.sr-only` to visually hide file inputs while keeping them focusable, and apply `.focus-within:ring-2` to the parent container/label to ensure keyboard users have visible focus states.
