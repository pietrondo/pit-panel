...
## 2026-09-09 - Accessible file upload inputs
**Learning:** Using `class="hidden"` (`display: none`) on an `<input type="file">` removes it completely from the accessibility tree and keyboard navigation flow, making it impossible for screen reader or keyboard-only users to focus and interact with the upload.
**Action:** Always use `.sr-only` to visually hide file inputs while keeping them focusable. Also, add `.focus-within:ring-2` (and related outline/offset utilities) on the wrapping `<label>` so keyboard users can see when the hidden input receives focus.
