        ...
        for name, svc in data["services"].items():
            ...
            if "mariadb" in image or "mysql" in image:
                return (
                    name,
                    "mysql",
                    _resolve("MYSQL_USER", cfg),
                    _resolve("MYSQL_PASSWORD", cfg) or _resolve("MYSQL_ROOT_PASSWORD", cfg),
                    _resolve("MYSQL_DATABASE", cfg),
                )
...
async def perform_app_backup(
    ...
    """Extracts backup logic from app_backup_run for reuse."""
...
async def scheduled_backup_loop() -> None:
    """Background task to run scheduled backups for all apps."""
    import asyncio
            ...
            if settings.backup_enabled:
                async with sessionmaker() as db:
                    result = await db.execute(
                        select(Subdomain).where(Subdomain.app_type.is_not(None))
                    )
